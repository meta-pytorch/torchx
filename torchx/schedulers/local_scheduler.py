...
class LocalScheduler(Scheduler[Mapping[str, CfgVal]]):
    ...
    **Compatibility**
    ...
    def __init__(
        ...
        # tmp log dirs this scheduler created (deleted on close);
        # user-provided `log_dir`s are never tracked here
        self._tmp_log_dirs: list[str] = []
    ...
    def _evict_lru(self) -> bool:
        ...
        for app_id, app in self._apps.items():
            if is_terminal(app.state):
                # strict `<`: on equal timestamps keep the first-seen
                # (oldest-inserted) app as the eviction candidate
                if app.last_updated < lru_time:
                    lru_time = app.last_updated
                    lru_app_id = app_id
    ...
    def _get_app_log_dir(self, app_id: str, cfg: Opts) -> str:
        """
        Returns the log dir for the app: ``cfg.log_dir`` when the user provided
        one, otherwise an autogenerated tmp dir. Tmp dirs are tracked in
        ``self._tmp_log_dirs`` and removed on ``close()``; user-provided dirs
        are never deleted.
        """
        ...
        if not base_log_dir:
            ...
            self._tmp_log_dirs.append(base_log_dir)
            log.info(
...
======================================================================
    ...
    def _to_popen_request(
        ...
        for role in app.roles:
            ...
            # build PATH locally, NOT into `role.env`; mutating the caller's
            # AppDef would re-prepend on every dryrun of the same app
            # (prepend extra_paths to PATH)
            path = _join_PATH(*self._extra_paths, role.env.get("PATH"))
            ...
            for replica_id in range(role.num_replicas):
                ...
                # the composed PATH is built before values.apply, so run it
                # through the same macro substitution the role's env got
                replica_role.env["PATH"] = values.substitute(path)
    ...
    def _cancel_existing(self, app_id: str) -> None:
        ...
        local_app = self._apps[app_id]
        local_app.close()
        local_app.set_state(AppState.CANCELLED)
...
class LogIterator:
    ...
    def _check_finished(self) -> None:
        # finished is sticky: once set (non-tail iteration, or a previously
        # observed terminal state) it must not flip back to unfinished
