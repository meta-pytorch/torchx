...
class LocalDirectorySchedulerTest(unittest.TestCase, LocalSchedulerTestUtil):
    ...
    def test_submit(self) -> None:
        ...
        self.assertEqual([], self.scheduler._tmp_log_dirs)
    ...
    def test_submit_cleanup(self) -> None:
        ...
        tmp_log_dirs = list(self.scheduler._tmp_log_dirs)
        self.assertEqual(1, len(tmp_log_dirs))
        self.scheduler.close()
        self.assertFalse(
            os.path.exists(tmp_log_dirs[0]),
            "close() must delete the autogenerated tmp log dir",
        )

    def test_close_preserves_user_log_dir(self) -> None:
        # tmp log dir (no log_dir cfg) followed by a user-provided log_dir:
        # close() must delete only the tmp dirs, never the user's log dir
        self.scheduler._get_app_log_dir("app1", Opts())
        self.scheduler._get_app_log_dir("app2", Opts())
        user_log_dir = join(self.test_dir, "user_logs")
        self.scheduler._get_app_log_dir("app3", Opts(log_dir=user_log_dir))
        os.makedirs(user_log_dir)

        tmp_base_dirs = list(self.scheduler._tmp_log_dirs)
        self.assertEqual(2, len(tmp_base_dirs))

        self.scheduler.close()

        self.assertTrue(os.path.exists(user_log_dir))
        for tmp_base_dir in tmp_base_dirs:
            self.assertFalse(os.path.exists(tmp_base_dir))
    ...
    def test_to_popen_request_path_idempotent(self) -> None:
        scheduler = LocalScheduler(
            session_name="test_session",
            image_provider_class=LocalDirectoryImageProvider,
            extra_paths=["/extra/bin"],
        )
        ...
        # extra_paths and cwd must be applied exactly once per request
        self.assertEqual(f"/extra/bin:/home/bob:{self.test_dir}", path_1)
        self.assertEqual(path_1, path_2)
        # and the caller's AppDef must not be mutated
        self.assertEqual({"PATH": "/home/bob"}, role.env)

    def test_to_popen_request_path_substitutes_macros(self) -> None:
        scheduler = LocalScheduler(
            session_name="test_session",
            image_provider_class=LocalDirectoryImageProvider,
            extra_paths=[join(macros.img_root, "bin")],
        )
        role = Role(name="foo", image=self.test_dir, env={"PATH": "/home/bob"})
        app = AppDef(name="_", roles=[role])
        cfg = {"log_dir": self.test_dir}
        ...
        # the PATH is composed outside values.apply so it must be run through
        # macro substitution like the rest of the role's env
        self.assertEqual(
            f"{self.test_dir}/bin:/home/bob:{self.test_dir}",
            path,
            "macros in the composed PATH must be substituted",
        )
                ...
                self.assertEqual(
                    {
                        ...
                        # cwd (the role's image dir) is appended to PATH in the
                        # request, without mutating `role.env`
                        "PATH": self.test_dir,
                        **role.env,
                    },
                    replica_param.env,
    ...
    def test_log_iterator(self) -> None:
        ...
        app = AppDef(name="test_app", roles=[role])
        app_id = self.scheduler.submit(app, cfg)
        # non-tail iteration reads only what is already written; wait for the
        # app to finish so all lines are present
        self.wait(app_id)
    ...
    def test_log_iterator_no_tail_running_app(self) -> None:
        role = Role(
            "role1",
            image=self.test_dir,
            entrypoint="sleep.sh",
        ...
        app = AppDef(name="test_app", roles=[role])
        app_id = self.scheduler.submit(app, Opts(log_dir=join(self.test_dir, "log")))
        ...
        reader = threading.Thread(
            target=lambda: logs.append(
                list(self.scheduler.log_iter(app_id, "role1", k=0, should_tail=False))
            ),
            daemon=True,
        )
        reader.start()
        ...
        self.assertFalse(
            reader.is_alive(),
            "non-tail log_iter of a RUNNING app must terminate at EOF",
        )
        self.assertEqual([[]], logs)  # sleep.sh logs nothing
        # don't leave the sleep child running until teardown
    ...
    def test_cancel_existing_updates_last_updated(self) -> None:
        app = _LocalAppDef("app0", self.test_dir)
        self.scheduler._apps["app0"] = app
        self.assertEqual(-1, app.last_updated)
        self.scheduler._cancel_existing("app0")
        self.assertEqual(AppState.CANCELLED, app.state)
        # must go through set_state so LRU eviction sees the cancel time
        self.assertGreater(app.last_updated, 0)

    def test_evict_lru_evicts_oldest_terminal_app(self) -> None:
        oldest = _LocalAppDef("oldest", self.test_dir)
        newest = _LocalAppDef("newest", self.test_dir)
        oldest.set_state(AppState.SUCCEEDED)
        newest.set_state(AppState.SUCCEEDED)
        oldest.last_updated = 1.0
        newest.last_updated = 2.0
        # insert oldest first so a bug that keeps the LAST-iterated terminal
        # app as the eviction candidate would evict `newest` instead
        self.scheduler._apps["oldest"] = oldest
        self.scheduler._apps["newest"] = newest

        self.assertTrue(self.scheduler._evict_lru())

        self.assertNotIn("oldest", self.scheduler._apps)
        self.assertIn("newest", self.scheduler._apps)

    def test_evict_lru_tie_breaks_to_first_inserted(self) -> None:
        first = _LocalAppDef("first", self.test_dir)
        second = _LocalAppDef("second", self.test_dir)
        first.set_state(AppState.SUCCEEDED)
        second.set_state(AppState.SUCCEEDED)
        # identical timestamps: the FIRST-inserted app must be evicted
        first.last_updated = 1.0
        second.last_updated = 1.0
        self.scheduler._apps["first"] = first
        self.scheduler._apps["second"] = second

        self.assertTrue(self.scheduler._evict_lru())

        self.assertNotIn("first", self.scheduler._apps)
        self.assertIn(
            "second",
            self.scheduler._apps,
            "on a timestamp tie the newer-inserted app must survive",
        )
    ...
    def test_get_gpu_device_count_no_nvidia_smi(self) -> None:
        # hosts without a GPU driver have no `nvidia-smi` binary at all
